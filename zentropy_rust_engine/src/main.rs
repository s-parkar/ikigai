...
fn main() {
    ...
    eprintln!("[Zentropy Rust Engine] Initializing Direct High-Speed Stream (Target Frames: {})...", total_est);
    ...
    cmd_dec_l.args(["-hwaccel", "cuda"]);
    if start_sec > 0.0 {
        cmd_dec_l.args(["-ss", &format!("{:.3}", start_sec)]);
    }
    cmd_dec_l.args(["-i", &lhs]);
    if let Some(dur) = dur_sec {
        cmd_dec_l.args(["-t", &format!("{:.3}", dur)]);
    }
    cmd_dec_l.args(["-f", "rawvideo", "-pix_fmt", "bgr24", "pipe:1"]);
    cmd_dec_l.stdout(Stdio::piped()).stderr(Stdio::null());

    let mut cmd_dec_r = Command::new(&ffmpeg_bin);
    cmd_dec_r.args(["-hwaccel", "cuda"]);
    if start_sec > 0.0 {
        cmd_dec_r.args(["-ss", &format!("{:.3}", start_sec)]);
    }
    cmd_dec_r.args(["-i", &rhs]);
    if let Some(dur) = dur_sec {
        cmd_dec_r.args(["-t", &format!("{:.3}", dur)]);
    }
    cmd_dec_r.args(["-f", "rawvideo", "-pix_fmt", "bgr24", "pipe:1"]);
    ...
    eprintln!("[Zentropy Rust Engine] Rayon Multi-Threaded SIMD Pipeline Active! Stitching frames...");
    ...
    println!("PROGRESS:{}|{}|{:.1}|{:.1}|{:.1}", count, count, final_fps, total_elapsed, 0.0);
}
